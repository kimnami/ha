...
int gen_instr_valid()
{
    return ((icode) == (I_NOP) || (icode) == (I_HALT) || (icode) == 
      ...
      (icode) == (I_MRMOVL) || (icode) == (I_LEAVE) || (icode) == (I_IADDL)
       || (icode) == (I_ISUBL) || (icode) == (I_ALU) || (icode) == (I_JMP)
...
int gen_srcB()
{
    return (((icode) == (I_ALU) || (icode) == (I_RMMOVL) || (icode) == 
        (I_MRMOVL)) ? (rb) : ((icode) == (I_IADDL) || (icode) == (I_ISUBL))
       ? (rb) : ((icode) == (I_PUSHL) || (icode) == (I_POPL) || (icode) == 
...
int gen_dstE()
{
    return ((((icode) == (I_RRMOVL)) & (cond)) ? (rb) : ((icode) == 
        (I_IRMOVL) || (icode) == (I_ALU)) ? (rb) : ((icode) == (I_IADDL)
         || (icode) == (I_ISUBL)) ? (rb) : ((icode) == (I_PUSHL) || (icode)
         == (I_POPL) || (icode) == (I_CALL) || (icode) == (I_RET)) ? 
      (REG_ESP) : ((icode) == (I_LEAVE)) ? (REG_ESP) : (REG_NONE));
}
...
int gen_aluA()
{
    return (((icode) == (I_RRMOVL) || (icode) == (I_ALU)) ? (vala) : (
        ...
        ) ? -4 : ((icode) == (I_RET) || (icode) == (I_POPL)) ? 4 : ((icode)
         == (I_LEAVE)) ? 4 : ((icode) == (I_IADDL) || (icode) == (I_ISUBL))
       ? (valc) : 0);
}
...
int gen_aluB()
{
    return (((icode) == (I_RMMOVL) || (icode) == (I_MRMOVL) || (icode) == 
        ...
        (I_LEAVE)) ? (valb) : ((icode) == (I_IADDL) || (icode) == (I_ISUBL)
